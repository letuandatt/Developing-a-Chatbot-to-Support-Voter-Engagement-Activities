...
class PatternConfig:
    ...
    SIGNATURE_PATTERNS = [
        ...
        re.compile(
            r"^\s*(Phạm Minh Chính|Nguyễn Xuân Phúc|Vũ Đức Đam|Nguyễn "
                    r"Tấn Dũng|Nguyễn Bắc Son|Nguyễn Văn Bình|Nguyễn Sinh Hùng)\s*$",
                    re.IGNORECASE),
        re.compile(r"^\s*\./\.\s*$"),
    ]
...
class PDFReader:
    ...
    def read_pdf(self, file_path: str) -> List[Dict]:
        ...
        try:
            ...
            doc.close()
            logging.info(f"Đã đọc {len(blocks)} blocks từ {file_path}")
            return blocks
        except Exception as e:
            logging.error(f"Lỗi đọc PDF {file_path}: {e}")
            raise
...
class MetadataExtractor:
    ...
    def extract_metadata(self, blocks: List[Dict], file_name: str):
        """Trích xuất metadata từ các blocks"""
        metadata = {
            "so_hieu": "",
            "loai_van_ban": "",
            "noi_ban_hanh": "",
        ...
        for i, block in enumerate(blocks):
            ...
            input_file = "../../data/CongThongTinDienTu/CongDien/metadata_congthongtindientu_congdien.jsonl"
            ...
            if type_found and type_block_index != -1:
                ...
                while j < len(blocks) and count_lookahead < max_lookahead:
                    ...
                    if metadata["loai_van_ban"] == "CÔNG ĐIỆN":
                        if not line or self.text_cleaner.is_header_footer(line) or self.text_cleaner.is_signature(line) or any(
                                p.match(line) for p in self.patterns.SECTION_PATTERNS.values()) or re.search(r"điện\s*:", line,
                                                                                               re.IGNORECASE):
                            break
                    else:
                        if not line or self.text_cleaner.is_header_footer(line) or self.text_cleaner.is_signature(line) or any(
    ...
    def parse_content(self, blocks: List[Dict], file_name: str) -> Dict:
        ...
        while i < len(cleaned_lines):
            ...
            if not content_started and any(
                    p.match(line) for p in
                    [self.patterns.SECTION_PATTERNS["chuong"], self.patterns.SECTION_PATTERNS["muc"]]):
...
class MainProcessor:
    ...
    def process_pdf_folder(self, folder_path: str, output_json: str):
...
if __name__ == "__main__":
    ...
    for folder_name in os.listdir(folder_to_process):
        ...
        if folder_name == "CongDien":
            ...
            main_processor.process_pdf_folder(folder_path, output_file_to_save)
