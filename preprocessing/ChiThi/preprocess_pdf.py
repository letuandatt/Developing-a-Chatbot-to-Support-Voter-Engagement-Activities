...
METADATA_PATTERNS = {
    ...
    "date": re.compile(
        ...
        re.IGNORECASE),
    "type": re.compile(r"^(CHỈ THỊ|NGHỊ ĐỊNH|THÔNG TƯ|QUYẾT ĐỊNH|LUẬT)\s*$", re.IGNORECASE),
}
...
def parse_pdf_content(blocks: List[Dict], file_name: str) -> Dict:
    ...
    while i < len(cleaned_lines):
        ...
        if content_started:
            ...
            # else: # Nội dung không thuộc cấu trúc nào? Bỏ qua hoặc xử lý khác
            #     print(f"Warning: Nội dung không xác định: {line}")
...
if __name__ == "__main__":
    folder_path = "../../data/CongThongTinDienTu/ChiThi"  # Giữ nguyên đường dẫn ví dụ
    output_json = "processed_documents.json"  # Giữ nguyên tên file output ví dụ
